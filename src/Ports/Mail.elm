...
navigationProgram :
    ...
    -> Platform.Program Never (Model model msg) (Msg msg)
navigationProgram onNavigation manifest =
    { init =
        navigationInit
            manifest.toJs
            manifest.fromJs
...
navigationInit :
    ...
    -> ((Value -> Msg msg) -> Sub (Msg msg))
    -> (msg -> model -> ( model, Mail msg ))
    -> (Location -> ( model, Mail msg ))
    -> Location
    -> ( Model model msg, Cmd (Msg msg) )
navigationInit toJs fromJs appUpdate toInitTuple location =
    let
        ( appModel, mail ) =
            toInitTuple location
    in
    ( { appModel = appModel
      , appUpdate = appUpdate
      , threads = Dict.empty
    ...
    case mail of
        ...
        Batch (mail :: rest) ->
            let
                ...
                ( model2, moreCmds ) =
                    handleMail ( model1, Batch rest )
            in
            ( model2, Cmd.batch [ newCmd, moreCmds ] )
